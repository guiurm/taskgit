...
import { existsSync as existsSync$1, mkdirSync as mkdirSync$1, writeFileSync as writeFileSync$1, unlinkSync } from 'node:fs';
...
if (!existsSync$1(TMP_DIR)) {
    mkdirSync$1(TMP_DIR);
}
if (!existsSync$1(TMP_PATCH_DIR)) {
    mkdirSync$1(TMP_PATCH_DIR);
}
class CacheStore {
    ...
    createPatchCache(file) {
        ...
        writeFileSync$1(fileChanges.originalState.cacheFilePath, fileChanges.originalState.content);
        writeFileSync$1(fileChanges.acceptedChanges.cacheFilePath, fileChanges.acceptedChanges.content);
        if (fileChanges.ignoredChanges)
            writeFileSync$1(fileChanges.ignoredChanges.cacheFilePath, fileChanges.ignoredChanges.content);
...
export { AppError, COMMIT_STANDARD_TYPES, CacheStore, ChangeLogService, ErrorHandler, ExternalServiceError, FileServiceError, GitDiffOutputFile, GitDiffService, GitService, GitServiceError, GitServiceFilesReport, GitServiceTagger, IS_DEV, IS_TEST, LOG_SPLITTER, MarkdownService, NAME, TMP_DIR, TMP_PATCH_DIR, VERSION, VERSION_NAME, createDirPath, createFileHash, exeCommand, existsPath, getFileStat, isDirectory, isFile, mf, parseTagsList, processFiles, resolvePath, rf, sha1, wf };
